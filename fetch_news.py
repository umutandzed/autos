...
def strip_google_news_url(url):
    ...
    try:
        ...
        if canonical_match:
            canonical_url = unescape(canonical_match.group(1))
        ...
        if amp_match:
            candidate = unescape(amp_match.group(1))
            if candidate:
                return candidate
        ...
        print(f"    Google News link cozme uyarisi: {exc}")
...
def fetch_page_image(url):
    ...
    try:
        resolved_url = strip_google_news_url(url)
        response = requests.get(
            resolved_url,
            timeout=HTTP_TIMEOUT_SECONDS,
            headers={
        ...
        if image_match:
            candidate = unescape(image_match.group(1))
